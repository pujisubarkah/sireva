  ...
  <div class="space-y-6 max-w-5xl mx-auto pb-10">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="px-6 py-5 border-b border-slate-200 bg-slate-50">
        <h1 class="text-xl font-bold text-slate-800">Laporan Sasaran Strategis</h1>
        <p class="text-sm text-slate-500 mt-1">Kelola dan evaluasi laporan capaian indikator sasaran strategis.</p>
      </div>
        ...
        <div v-else class="overflow-x-auto border border-slate-200 rounded-xl">
          ...
          >
            ...
            <template #cell-sasaranName="{ row }">
              <div class="line-clamp-2 text-xs text-slate-600" :title="getSasaranName(row.sasaranId)">
                {{ getSasaranName(row.sasaranId) }}
              </div>
            </template>
          ...
          <div class="flex-1 overflow-y-auto p-4 sm:p-8 bg-slate-100">
            <div class="bg-white max-w-3xl mx-auto shadow-sm border border-slate-200 rounded-lg p-8 sm:p-12">
              ...
              <div class="text-center pb-8 border-b-2 border-slate-800 mb-8">
                <h1 class="text-2xl font-black text-slate-900 uppercase tracking-tight">Laporan Sasaran Strategis</h1>
              ...
              <div class="space-y-8 text-slate-800">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div class="md:col-span-3 font-medium text-lg leading-relaxed">{{ getSasaranName(viewingLaporan.sasaranId) }}</div>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div class="text-sm font-bold text-slate-500 uppercase tracking-wider">Indikator Kinerja</div>
                  <div class="md:col-span-3 font-bold text-xl text-blue-700 leading-relaxed">{{ getIndikatorName(viewingLaporan.indikatorId) }}</div>
                </div>
...
const { data: laporanList, isValidating: loading, mutate: mutateLaporan } = useSWRV('/api/laporan-sasaran-strategis', fetcher);
...
const filteredLaporanList = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    list = list.filter((l: any) => {
      const sasaran = getSasaranName(l.sasaranId).toLowerCase();
...
async function handleSubmit() {
  ...
  try {
    const res = await $fetch('/api/laporan-sasaran-strategis', {
      method: 'POST',
      body: form.value
    });
...
async function handleDelete(id: number) {
  ...
  try {
    await $fetch('/api/laporan-sasaran-strategis', {
      method: 'DELETE',
      body: { id }
    });
    mutateLaporan();
...
function getSasaranName(id: number) {
  ...
  const match = sasaranList.value.find((s: any) => s.id === id);
  return match ? match.sasaranText : '-';
}
